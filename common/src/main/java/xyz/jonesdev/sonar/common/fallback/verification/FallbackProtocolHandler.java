...
public final class FallbackProtocolHandler extends FallbackVerificationHandler {
  ...
  public void handle(final @NotNull FallbackPacket packet) {
    if (packet instanceof TransactionPacket) {
      ...
      // Only perform the SetHeldItem check on Java players since the Bedrock protocol
      // breaks this check.
      ...
      // TODO: Removed slot check, because of false positives
      // Also check if the player sent an invalid slot which is impossible by vanilla
      // protocol
      // checkState(slotId >= 0 && slotId <= 8, "slot out of range: " + slotId);
      // Check if the player sent a duplicate slot packet which is impossible by
      // vanilla protocol
      ...
      if (expectedSlotId != -1
          ...
          // This can false flag if a player spams these packets, which is why we don't
          // fail for this
  ...
  /**
   * Uses Transaction (ping) packets to check for an immediate, legitimate
   * response from the client
   * <br>
   * <a href="https://wiki.vg/Protocol#Ping_.28configuration.29">Wiki.vg - Ping
   * (configuration)</a>
   ...
   * <br>
   * <a href="https://wiki.vg/Protocol#Set_Held_Item_.28serverbound.29">Wiki.vg -
   * SetHeldItem (play)</a>
   */
  private void sendSetHeldItem() {
    // Send an invalid HeldItemChange packet to the player to see if the player
    // responds at all
    user.delayedWrite(INVALID_HELD_ITEM_SLOT);
    // Increment the player's slot by a random slot, and then modulo it by the
    // maximum slot (8)
    expectedSlotId = (currentClientSlotId + 1 + RANDOM.nextInt(7)) % 8;
    // Send two SetHeldItem packets with the same slot to check if the player
    // responds with the correct slot.
    // By vanilla protocol, the client does not respond to duplicate SetHeldItem
    // packets.
    // We can take advantage of this by sending two packets with the same content to
  ...
  /**
   * Uses EntityAnimation packets to check for the excepted Animation (SwingArm)
   * from the client.
   * <br>
   * <a href="https://wiki.vg/Protocol#Entity_Animation">Wiki.vg -
   * EntityAnimation</a>
